...
public:
  ControlLoopNode() : Node("control_loop") {
    ...
    motor_cmd_publisher_ =
        this->create_publisher<ros381_interfaces::msg::Float2>("motor_cmd", 10);
...
private:
  ...
  void control_loop() {
    if (odom_initialized_) {
      ...
      v_right_ =
          std::clamp(v_ref_ + w_ref_ * L_ * 0.5, -MOTOR_V_MAX_, MOTOR_V_MAX_);
      v_left_ =
          std::clamp(v_ref_ - w_ref_ * L_ * 0.5, -MOTOR_V_MAX_, MOTOR_V_MAX_);
  ...
  void rotate() {
    if (movement_state_ == 0) {
      ...
      slowing_coeff_ =
          std::clamp(pow(fabs(phi_error_) / (starting_angle_ + stopping_angle_),
                         2.0 / 3.0),
                     0.0, 1.0);
  ...
  void go_to_xy() {
    ...
    case 2:
      ...
      starting_distance_ =
          5 * pow(v_max_temp_, 1.5) / 3 / sqrt(j_max_temp_) * starting_coeff_v_;
      stopping_distance_ =
          5 * pow(v_max_temp_, 1.5) / 3 / sqrt(J_MAX_STOP_) * stopping_coeff_v_;

      slowing_coeff_ = std::clamp(
          pow(distance_ / (starting_distance_ + stopping_distance_), 2.0 / 3.0),
          0.0, 1.0);
    ...
    case 3:
      ...
      v_ref_ =
          synthesis_7(distance_proj_ * direction_, v_base_, a_, j_max_temp_,
                      stopping_distance_, v_max_temp_, V_MIN_, dt_);
      w_ref_ =
          P_w_ *
          std::clamp((distance_ - D_SHORT_TOL_) / (D_LONG_TOL_ - D_SHORT_TOL_),
                     0.0, 1.0) *
          phi_error_;
  ...
  double synthesis_7(double distance, double velocity, double acceleration,
    ...
    if (abs_distance <= stopping_distance) {
      ...
      v_ref = v_max * (35.0f * pow(x, 4) - 84.0f * pow(x, 5) +
