    ...
    match tactic_state:
        ...
        case 11:
            if get_side() == 1:  # plava
                ...
                move_to_xy(x=-0.345, y=0.15, dir=1)
            tactic_state = 12
        case 12:
            if move_success() or move_stacked() or move_failed():
                tactic_state = 13
        ...
        case 60:
            if get_side() == 1:
                move_to_xy(x=-0.42, y=-0.45, dir=-1)  # x???
            else:
        ...
        case 118:
            ...
            move_to_xy(x=-0.92, y=-0.62, dir=-1)
            tactic_state = 120
        case 120:
            if move_success() or move_stacked() or move_failed():
                tactic_state = 122
        ...
        case 124:
            ...
            if get_side() == 1:
                direction = -1
                move_to_xy(x=-0.25, y=-0.785, dir=direction)  # x+
            else:
                direction = 1
                move_to_xy(x=-0.2, y=-0.785, dir=direction)
            tactic_state = 125
        case 125:
            if move_success() or move_stacked() or move_failed():
                tactic_state = 130
        ...
        case 140:
            if get_side() == 1:
                move_cursor(dist=0.555, phi=cursor_phi)
            else:
        ...
        case 420:
            if get_side() == 1:
                move_on_angle(dist=0.34, dir=-1, phi=math.pi)
            else:
                move_on_angle(dist=0.34, dir=-1, phi=math.pi)
            tactic_state = 425
        case 425:
        ...
        # Stackuje se u zid
        case 440:
            move_on_angle(dist=0.1, dir=-1, phi=math.pi, v_max=0.2)
            tactic_state = 445
        case 445:
            if move_stacked():
                if get_side() == 1:
        ...
        case 500:
            if get_side() == 1: # plava
                ...
                move_to_xy(x=0.065, y=-0.55, dir=1)
            tactic_state = 505
        case 505:
            if move_success() or move_stacked() or move_failed():
                tactic_state = 510
