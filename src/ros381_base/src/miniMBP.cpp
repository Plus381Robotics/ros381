...
public:
  MiniMBP () : Node ("minimbp")
  {
    ...
    move_action_server_ = rclcpp_action::create_server<Move> (
        this, "move", std::bind (&MiniMBP::handle_goal, this, _1, _2),
        std::bind (&MiniMBP::handle_cancel, this, _1),
        std::bind (&MiniMBP::handle_accepted, this, _1));

    obstacle_sub_ = this->create_subscription<example_interfaces::msg::UInt8> (
        "obstacle_status", 10,
        std::bind (&MiniMBP::callback_obstacle, this, _1));

    obstacle_dir_pub_ = this->create_publisher<example_interfaces::msg::Int8> (
        "obstacle_direction", 10);

    mini_mbp_pub_ = this->create_publisher<ros381_interfaces::msg::MiniMBP> (
        "miniMBP", 10);

    timer_ = this->create_wall_timer (
        std::chrono::microseconds (period_),
        std::bind (&MiniMBP::publish_mini_mbp, this));
...
private:
  ...
  rclcpp::Subscription<example_interfaces::msg::UInt8>::SharedPtr
      obstacle_sub_;
  rclcpp::Publisher<example_interfaces::msg::Int8>::SharedPtr
      obstacle_dir_pub_;
  ...
  void
  callback_obstacle (const example_interfaces::msg::UInt8::SharedPtr msg)
  {
               ...
               std::shared_ptr<const Move::Goal> goal)
  {
    ...
    if (current_goal_handle_ && current_goal_handle_->is_active ())
      {
        RCLCPP_INFO (get_logger (),
                     "A goal is already active—rejecting new one.");
    ...
    switch (goal->type)
      ...
      case -2:
        x_ref_ = x_base_;
        y_ref_ = y_base_;
        phi_ref_ = atan2 (goal->y - y_base_, goal->x - x_base_)
                   + (goal->direction - 1) * M_PI * 0.5;
        RCLCPP_INFO (this->get_logger (),
                     "Rotate to XY:\nx = %.4f, y = %.4f, phi_ref = %.4f",
                     goal->x, goal->y, phi_ref_);
        reg_type_ = -1;
        break;
      case 1:
        x_ref_ = goal->x;
  ...
  void
  move (const std::shared_ptr<GoalHandleMove> goal_handle)
  {
  ...
  publish_mini_mbp ()
  {
    ...
    msg.v_max_100 = static_cast<uint8_t> ((v_max_temp_ / V_MAX_) * 100.0);
    msg.w_max_10 = static_cast<uint8_t> ((w_max_temp_ / W_MAX_) * 10.0);
    msg.tol_perc = static_cast<uint8_t> ((d_tol_perc_ + phi_tol_perc_) / 2);
    msg.coeff = static_cast<uint8_t> ((starting_coeff_v_ + stopping_coeff_v_
                                       + starting_coeff_w_ + stopping_coeff_w_)
                                      / 4);
